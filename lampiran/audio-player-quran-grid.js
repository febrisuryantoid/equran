...
function injectAudioFullBtn() {
  ...
  if (__SURAT_LIST.length < 1) {
    __SURAT_LIST = Array.from({ length: 114 }, (_, t) => pad3(t + 1));
...
async function loadAudioMetaByIndex(idx) {
  let e = __SURAT_LIST[idx],
      url = `https://equran.id/api/v2/surat/${e}`,
      o = await fetch(url).then(t => t.json()).catch(() => null);
...
async function playFullSurat() {
  ...
  updateGridPlayIcons(); // reset grid icon
}
  ...
  __AUDIO_PLAYER_POPUP.innerHTML = `
    <div class="audioPlayerCard">
      <div class="audio-col1">
        <button class="audio-btn-playpause" title="Putar/Jeda"><i class="fa-solid fa-${__QURAN_AUDIO && __QURAN_AUDIO.paused ? "play" : "pause"}"></i></button>
      </div>
...
async function getTafsirPerAyat(nomorSurat) {
  ...
  let url = `https://equran.id/api/v2/tafsir/${pad3(nomorSurat)}`;
  try {
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    let url = `https://equran.id/api/v2/surat/${pad3(nomorSurat)}`;
